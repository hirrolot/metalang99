...
#define EPILEPSY_PRIV_EVAL_MATCH_TRIVIAL_CALL_HOOK() EPILEPSY_PRIV_EVAL_MATCH_TRIVIAL_CALL
#define EPILEPSY_PRIV_EVAL_MATCH_CALL_OP_HOOK()      EPILEPSY_PRIV_EVAL_MATCH_CALL_OP
...
#define EPILEPSY_PRIV_EVAL(...)                                                                    \
    EPILEPSY_PRIV_EVAL_REC_UNROLL(                                                                 \
        EPILEPSY_PRIV_EVAL_AUX(EPILEPSY_PRIV_EVAL_REC_STOP, (~), __VA_ARGS__))

#define EPILEPSY_PRIV_EVAL_AUX(k, k_cx, ...)                                                       \
    EPILEPSY_PRIV_EVAL_MATCH(                                                                      \
        k, k_cx, EPILEPSY_PRIV_EVAL_ACC_EMPTY(), __VA_ARGS__ EPILEPSY_PRIV_EVAL_TERM_END(), ~)
...
#define EPILEPSY_PRIV_EVAL_MATCH_trivial_call(k, k_cx, acc, tail, op, ...)                         \
    EPILEPSY_PRIV_EVAL_REC_CONTINUE(                                                               \
        EPILEPSY_PRIV_EVAL_AUX_HOOK,                                                               \
        (EPILEPSY_PRIV_EVAL_MATCH_CALL_OP_HOOK, (k, k_cx, acc, tail, op)),                         \
        EPILEPSY_PRIV_EVAL_PATCH_TERMS_WITH_COMMAS(__VA_ARGS__))

#define EPILEPSY_PRIV_EVAL_MATCH_call(k, k_cx, acc, tail, op, ...)                                 \
    EPILEPSY_PRIV_EVAL_REC_CONTINUE(                                                               \
        EPILEPSY_PRIV_EVAL_AUX_HOOK,                                                               \
        (EPILEPSY_PRIV_EVAL_MATCH_TRIVIAL_CALL_HOOK, (k, k_cx, acc, tail, (__VA_ARGS__))), op,     \
        EPILEPSY_PRIV_AUX_EMPTY())

#define EPILEPSY_PRIV_EVAL_MATCH_v(k, k_cx, acc, tail, ...)                                        \
    EPILEPSY_PRIV_EVAL_MACHINE_OP_CONTINUE(                                                        \
        k, k_cx, EPILEPSY_PRIV_EVAL_ACC_EXTEND(acc, __VA_ARGS__), tail)

#define EPILEPSY_PRIV_EVAL_MATCH_end(k, k_cx, acc, _tail, _)                                       \
    EPILEPSY_PRIV_EVAL_MACHINE_OP_STOP(k, k_cx, acc)

#define EPILEPSY_PRIV_EVAL_MATCH_TRIVIAL_CALL(k, k_cx, acc, tail, args, evaluated_op)              \
    EPILEPSY_PRIV_EVAL_MATCH(                                                                      \
        k, k_cx, acc,                                                                              \
        call(evaluated_op, EPILEPSY_PRIV_AUX_UNPARENTHESISE(args))                                 \
            EPILEPSY_PRIV_EVAL_CONTROL_UNWRAP(tail))

#define EPILEPSY_PRIV_EVAL_MATCH_CALL_OP(k, k_cx, acc, tail, evaluated_op, ...)                    \
    EPILEPSY_PRIV_EVAL_MACHINE_OP_CONTINUE(                                                        \
