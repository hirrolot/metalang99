#ifndef EPILEPSY_PRIV_UINT_LESSER_EQUAL_H
#define EPILEPSY_PRIV_UINT_LESSER_EQUAL_H

#include <epilepsy/priv/aux.h>
#include <epilepsy/priv/uint/eq.h>
#include <epilepsy/priv/uint/lesser.h>

#define EPILEPSY_PRIV_UINT_LESSER_EQUAL(x, y)                                                      \
    EPILEPSY_PRIV_AUX_IF(                                                                          \
        EPILEPSY_PRIV_UINT_EQ(x, y), EPILEPSY_PRIV_UINT_LESSER_EQUAL_TRUE,                         \
        EPILEPSY_PRIV_UINT_LESSER)                                                                 \
    (x, y)

#define EPILEPSY_PRIV_UINT_LESSER_EQUAL_TRUE(_x, _y) 1

#endif // EPILEPSY_PRIV_UINT_LESSER_EQUAL_H
