...
#define MACROLOP_PRIV_EVAL_MATCH_REORDER_TRIVIAL_CALL_ARGS_HOOK()                                  \
    MACROLOP_PRIV_EVAL_MATCH_REORDER_TRIVIAL_CALL_ARGS
...
#define MACROLOP_PRIV_EVAL_MATCH(k, k_cx, acc, head, ...)                                          \
...
#define MACROLOP_PRIV_EVAL_MATCH_call(k, k_cx, acc, tail, op, ...)                                 \
    MACROLOP_PRIV_EVAL_REC_CONTINUE(                                                               \
        ...
        (MACROLOP_PRIV_EVAL_MATCH_REORDER_TRIVIAL_CALL_ARGS_HOOK,                                  \
         (k, k_cx, acc, tail, (__VA_ARGS__))),                                                     \
        op, MACROLOP_PRIV_EVAL_AUX_EMPTY())
...
#define MACROLOP_PRIV_EVAL_MATCH_REORDER_TRIVIAL_CALL_ARGS(k, k_cx, acc, tail, args, evaluated_op) \
    MACROLOP_PRIV_EVAL_MATCH_trivial_call(                                                         \
        k, k_cx, acc, tail, evaluated_op, MACROLOP_PRIV_EVAL_AUX_UNPARENTHESISE(args))

#define MACROLOP_PRIV_EVAL_MATCH_CALL_OP(k, k_cx, acc, tail, evaluated_op, ...)                    \
    MACROLOP_PRIV_EVAL_REC_CONTINUE(                                                               \
        MACROLOP_PRIV_EVAL_MATCH_HOOK, (k, k_cx), acc,                                             \
        evaluated_op(__VA_ARGS__) MACROLOP_PRIV_EVAL_AUX_UNPARENTHESISE(tail))
