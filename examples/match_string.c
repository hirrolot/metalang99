...
#define branchParseAux(...)      E_call(branchParseAux, __VA_ARGS__)
...
#define branchParse_IMPL(x) branchParseAux(E_unparenthesiseUnevaluated(v(x)))
#define branchParseAux_IMPL(...)                                                                   \
    branch(E_variadicsHead(v(__VA_ARGS__)), E_variadicsTail(v(__VA_ARGS__)))
