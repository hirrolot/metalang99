...
<term> ::= "EPILEPSY_CALL(" <op> "," { <term> }* ")"
         ...
         | "EPILEPSY_FATAL(" <ident> "," <string-literal> ")"
...
\begin{figure}
    ...
    \begin{align*}
        ...
        (dbg): & \ \langle K; F; A; \texttt{EPILEPSY\_DBG}(\overline{tok}) \ \overline{t} \rangle \to halt(\overline{tok}) \\
        (error): & \ \langle K; F; A; \texttt{EPILEPSY\_FATAL}(ident, str) \ \overline{t} \rangle \to \\
            & \ \ \ \ halt(\texttt{!Epilepsy error} \ \texttt{(}ident\texttt{):} \ str) \\
