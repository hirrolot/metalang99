...
\begin{minted}{bnf}
<eval> ::= "METALANG99_eval(" <term-seq> ")" ;

<term> ::=
      "METALANG99_call(" <op> "," <term-seq> ")"
    | "METALANG99_callTrivial(" <ident> "," <pp-token-list> ")"
    | "METALANG99_abort(" <term-seq> ")"
...
\begin{itemize}
    ...
    \item \texttt{METALANG99\_call} accepts \texttt{op} either as an identifier or as a term that
    reduces to an identifier.
    \item \texttt{METALANG99\_callTrivial} accepts an operation strictly as an identifier.
\end{itemize}

The \texttt{METALANG99\_call} syntax hurts IDE support (no parameters documentation highlighting) and is also
less natural. The workaround is to define a wrapper around an implementation macro like this:

\begin{minted}{c}
/// The documentation string.
#define F(a, b, c) METALANG99_call(F, a, b, c)
#define F_IMPL(a, b, c) // The implementation.
\end{minted}

Then \texttt{F} can be conveniently called as \texttt{F(v(1), v(2), v(3))}.
...
\begin{notation}[Sequence]
    \begin{itemize}
        ...
        \begin{itemize}
            \item Metalang99 terms: \texttt{v(abc), METALANG99\_call(F, v(123)), v(u 8 9)}
...
\begin{align*}
    \texttt{METALANG99\_eval}(\texttt{METALANG99\_call}(X, \texttt{v}(\texttt{CALL\_X}))) & \ \underrightarrow{(start)} \\
    \langle halt; fappend; (); \texttt{METALANG99\_call}(X, \texttt{v}(\texttt{CALL\_X})) \rangle & \ \underrightarrow{(args)} \\
    ...
    \langle halt; fappend; (); \texttt{METALANG99\_call}(\texttt{CALL\_X}, \texttt{v}(123)) \rangle & \ \underrightarrow{(args)} \\
    ...
    \langle halt; fappend; (); \texttt{METALANG99\_call}(X, \texttt{v}(\texttt{ID})) \rangle & \ \underrightarrow{(args)} \\
    ...
    \langle halt; fappend; (); \texttt{METALANG99\_call}(\texttt{ID}, \texttt{v}(123)) \rangle & \ \underrightarrow{(args)} \\
...
\item Consider this scenario:
    \begin{itemize}
        \item You call \texttt{F(1, 2, 3)}
        \item It gets expanded by the preprocessor (not by Metalang99)
        \item Its expansion contains \texttt{F}
    \end{itemize}
Then \texttt{F} gets blocked \cite{Blueprinting} by the preprocessor, i.e. Metalang99 cannot handle ordinary
