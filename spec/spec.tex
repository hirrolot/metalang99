...
<term> ::= "epCall(" <op> "," { <term> }* ")"
         | "epDbg(" <preprocessor-token-list> ")"
         | "epFatal(" <ident> "," <preprocessor-token-list> ")"
...
\begin{itemize}
    ...
    except for \texttt{epEval}, \texttt{epCall}, and \texttt{v}.
    \item \texttt{epCall} accepts \texttt{op} either as an identifier or as a non-empty
    sequence of terms that reduces to an identifier.
    \item \texttt{epCall} accepts arguments of \texttt{op} without a separator.
\end{itemize}

The \texttt{epCall} syntax hurts IDE support: bad code formatting, no parameters
documentation highlighting, et cetera. The workaround is to define a wrapper around
an implementation macro like this:

\begin{minted}{c}
/// The documentation string.
#define FOO(a, b, c) epCall(FOO, a b c)
...
\begin{minted}{c}
/// The documentation string.
#define FOO(a, b, c, ...) epCall(FOO, a b c __VA_ARGS__)
#define FOO_IMPL(a, b, c, ...) // The implementation.
\end{minted}
...
\begin{notation}[Sequence]
    \begin{itemize}
        ...
        \begin{itemize}
            \item Epilepsy terms: \texttt{v(abc) epCall(FOO, v(123)) v(u 8 9)}
...
\begin{figure}
    ...
    \begin{align*}
        ...
        (op): & \ \langle K; F; A; \texttt{epCall}(\overline{t}, \overline{a}) \ \overline{t'} \rangle \to \langle \langle K; F; A; \texttt{epCall}(?, \overline{a}) \ \overline{t'} \rangle; fappend; (); \overline{t} \rangle \\
        (args): & \ \langle K; F; A; \texttt{epCall}(ident, \overline{a}) \ \overline{t} \rangle \to \\
            & \ \ \ \ \langle \langle K; F; A; ident\texttt{\_IMPL}(?) \ \overline{t} \rangle; fcomma; (); \overline{a} \rangle \\
        (dbg): & \ \langle K; F; A; \texttt{epDbg}(\overline{tok}) \ \overline{t} \rangle \to halt(\ldots) \\
        (fatal): & \ \langle K; F; A; \texttt{epFatal}(ident, \overline{tok}) \ \overline{t} \rangle \to halt(\ldots) \\
        (end): & \ \langle K; F; A; () \rangle \to K(A) \\
        (start): & \ \texttt{epCall}(\overline{t}) \to \langle halt; fappend; (); \overline{t} \rangle
    \end{align*}
    \normalsize
...
\begin{itemize}
    ...
    \texttt{epCall} -- it makes easier to follow the convention that all implementations
...
See how \texttt{epCall(X, v(CALL\_X))} is evaluated:

\begin{example}[Evaluation of terms]
\begin{align*}
    \texttt{epEval}(\texttt{epCall}(X, \texttt{v}(\texttt{CALL\_X}))) & \to (start) \\
    \langle halt; fappend; (); \texttt{epCall}(X, \texttt{v}(\texttt{CALL\_X})) \rangle & \to (args) \\
    ...
    \langle halt; fappend; (); \texttt{epCall}(\texttt{CALL\_X}, \texttt{v}(123)) \rangle & \to (args) \\
    ...
    \langle halt; fappend; (); \texttt{epCall}(X, \texttt{v}(ID)) \rangle & \to (args) \\
    ...
    \langle halt; fappend; (); \texttt{epCall}(ID, \texttt{v}(123)) \rangle & \to (args) \\
...
\begin{itemize}
    \item The Epilepsy version: \texttt{X(CALL\_X)} expands to \texttt{epCall(CALL\_X, v(123))}.
...
macro recursion; you must use \texttt{epCall} to be sure that recursive calls
