...
Macros are used to abstract frequently occurring syntactical structures so that you can write code that _reflects_ the problem domain. However, metaprogramming in C is utterly castrated: we cannot even operate with control flow, integers, unbounded sequences, and compound data structures, thereby throwing a lot of hypothetically useful metaprograms out of scope.
