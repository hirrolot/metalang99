...
#define DIV(item, acc) v(acc / item)
#define SUB(acc, item) v(acc - item)

EPILEPSY_ASSERT_EQ(
    EPILEPSY_ListFoldr(EPILEPSY_List(v(2) v(5) v(3)), v(DIV), v(60)), v(60 / 2 / 5 / 3));
EPILEPSY_ASSERT_EQ(
    EPILEPSY_ListFoldl(EPILEPSY_List(v(3) v(4) v(1)), v(SUB), v(10)), v(10 - 3 - 4 - 1));
EPILEPSY_ASSERT_EQ(
    EPILEPSY_ListFoldl1(EPILEPSY_List(v(2280) v(8) v(5) v(3)), v(DIV)), v(2280 / 8 / 5 / 3));
